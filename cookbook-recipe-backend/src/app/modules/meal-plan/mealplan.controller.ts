...
const toggleAisleItemBuyStatus = catchAsync(async (req: Request, res: Response) => {
    const { planId, itemName, isPurchased } = req.body;
    const result = await MealService.toggleAisleItemBuyStatus(planId, itemName, isPurchased);
    sendResponse(res, {
        statusCode: 200,
        success: true,
        data: result,
        message: "Aisle item buy status updated successfully!",
    });
});

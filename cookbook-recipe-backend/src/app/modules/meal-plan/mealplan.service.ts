...
const addPlaneRecipes = async (query: {
    ...
    plan.weekendPrepAdvice = undefined;
    plan.fullAiPlanData = undefined;
    plan.groceryListAdvice = undefined;
        ...
        for (const recipe of day.recipes) {
            ...
            for (const ingredient of recipe.ingredients) {
                if (ingredient._id.toString() === ingredientId) {
                    ...
                    // Sync to fullAiPlanData.complete_grocery_list
                    const ingName = (ingredient.ingredient || "").toLowerCase();
                    if (plan.fullAiPlanData && plan.fullAiPlanData.complete_grocery_list) {
                        for (const dept of (plan.fullAiPlanData.complete_grocery_list as any[])) {
                            if (dept.items) {
                                for (const item of (dept.items as any[])) {
                                    const itemName = (item.name || item.ingredient || "").toLowerCase();
                                    if (itemName && (ingName.includes(itemName) || itemName.includes(ingName))) {
                                        item.isPurchased = ingredient.buy;
                                        item.is_purchased = ingredient.buy;
                                    }
                                }
                            }
                        }
                        plan.markModified('fullAiPlanData');
                    }
...
const generateWeeklyGroceryList = async (planId: string) => {
    ...
    if (!plan.fullAiPlanData || !plan.fullAiPlanData.days || plan.fullAiPlanData.days.length === 0) {
    ...
    // Sync isPurchased state from plan.data ingredients to complete_grocery_list items before returning
    if (plan.fullAiPlanData && plan.fullAiPlanData.complete_grocery_list) {
        const checkedNames = new Set<string>();
        for (const day of plan.data) {
            for (const recipe of (day.recipes as any[])) {
                if (recipe.ingredients) {
                    for (const ing of recipe.ingredients) {
                        if (ing.buy) {
                            checkedNames.add((ing.ingredient || "").toLowerCase());
                        }
                    }
                }
            }
        ...
        for (const dept of (plan.fullAiPlanData.complete_grocery_list as any[])) {
            if (dept.items) {
                for (const item of (dept.items as any[])) {
                    const itemName = (item.name || item.ingredient || "").toLowerCase();
                    if (item.isPurchased === undefined && item.is_purchased === undefined) {
                        let isMatched = false;
                        for (const checkedStr of checkedNames) {
                            if (checkedStr.includes(itemName) || itemName.includes(checkedStr)) {
                                isMatched = true;
                                break;
                            }
                        }
                        item.isPurchased = isMatched;
                        item.is_purchased = isMatched;
                    } else {
                        item.isPurchased = item.isPurchased || item.is_purchased || false;
                        item.is_purchased = item.isPurchased;
                    }
                }
            }
        }
...
const toggleAisleItemBuyStatus = async (planId: string, itemName: string, isPurchased: boolean) => {
    const plan = await MealPlanWeek.findById(planId);
    if (!plan) throw new ApiError(404, "Meal Plan not found!");

    const searchStr = (itemName || "").toLowerCase().trim();

    // 1. Update plan.data recipe ingredients matching itemName
    if (searchStr.length > 0) {
        for (const day of plan.data) {
            for (const recipe of (day.recipes as any[])) {
                if (recipe.ingredients) {
                    for (const ingredient of recipe.ingredients) {
                        const ingStr = (ingredient.ingredient || "").toLowerCase();
                        if (ingStr.includes(searchStr) || searchStr.includes(ingStr)) {
                            ingredient.buy = isPurchased;
                        }
                    }
                }
            }
    ...
    // 2. Update plan.fullAiPlanData.complete_grocery_list
    if (plan.fullAiPlanData && plan.fullAiPlanData.complete_grocery_list) {
        for (const dept of (plan.fullAiPlanData.complete_grocery_list as any[])) {
            if (dept.items) {
                for (const item of (dept.items as any[])) {
                    const nameStr = (item.name || item.ingredient || "").toLowerCase().trim();
                    if (nameStr.includes(searchStr) || searchStr.includes(nameStr)) {
    ...
    await plan.save();
    return { success: true, isPurchased };
};

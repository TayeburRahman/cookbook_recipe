...
const getRecipesForYou: RequestHandler = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user;
    const result = await DashboardService.getRecipesForYou(user as IReqUser, req.query);
    sendResponse(res, {
      statusCode: 200,
      success: true,
      message: `Get successfully`,
...
const postScoreReview = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const result = await DashboardService.postScoreReview(user, recipeId, ratting);
    sendResponse(res, {
      statusCode: 200,
      success: true,

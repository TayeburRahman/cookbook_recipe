...
const getRecipesForYou = async (user: IReqUser, query?: any) => {
  const { userId } = user;

  const userDb = (await User.findById(userId).select('helgth_goal').lean()) as unknown as IUser;
  if (!userDb || !userDb.helgth_goal) return [];

  const limit = Number(query?.limit) || 20;

  const goalFilter = Array.isArray(userDb.helgth_goal)
    ? { $in: userDb.helgth_goal }
    : userDb.helgth_goal;

  const recipes = await Recipe.find({
    weight_and_muscle: goalFilter,
  })
